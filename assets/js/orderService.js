...
const OrderService = {
  ...
  /* ---------- CLEAR ALL / EXPORT / IMPORT (LocalStorage) ---------- */

  /** Xóa toàn bộ đơn hàng khỏi LocalStorage */
  clearAllOrders() {
    localStorage.removeItem(ORDERS_KEY);
  },

  /** Xuất toàn bộ đơn hàng ra file JSON để tải về máy */
  exportOrdersToJson() {
    const orders = OrderService.getAllOrders();
    ...
    a.href = url;
    a.download = `cafe-orders-${Date.now()}.json`;
  ...
  /**
   * Nhập danh sách đơn hàng từ file JSON (khôi phục dữ liệu).
   * mode: "replace" (ghi đè toàn bộ) | "merge" (gộp thêm, tránh trùng id)
   */
  importOrdersFromJson(jsonText, mode = "replace") {
    let imported;
    try {
    ...
    if (!Array.isArray(imported)) {
      throw new Error("Nội dung JSON phải là một danh sách đơn hàng.");
    }

    if (mode === "merge") {
      const current = OrderService.getAllOrders();
      ...
      OrderService.saveAllOrders(merged);
      return merged.length;
    }

    OrderService.saveAllOrders(imported);
